...
function QlobberFSQ(options)
{
    ...
    this._dehnd = '__fsq_dehnd' + crypto.randomBytes(16).toString('base64');
    ...
    {
        if (this._stopped && this._watching)
        {
    ...
    function make_info_handler(cache, cache2, singles2)
    {
        return function (info, next)
        {
            ...
            if (info.single)
            {
                for (i = 0; i < handlers.length; i += 1)
                {
                    ...
                    if ((!ths._do_dedup) || (h[ths._dedup] !== dedup))
                    {
                        h[ths._dedup] = dedup;
                        if ((!h.accept) || h.accept(info))
                        {
                            call_handlers([h], info, cb);
                            return;
                        }
                    }
                }
            ...
            {
                call_handlers(handlers.filter(function (h)
                {
                    if ((!ths._do_dedup) || (h[ths._dedup] !== dedup))
                    {
                        h[ths._dedup] = dedup;
    ...
    function make_fname_handler(cache, cache2, singles2)
    {
        var info_handler = make_info_handler(cache, cache2, singles2);
    ...
    function handle(files, cache, cache2, make_handler, cb)
    {
        ...
        async.eachLimit(files,
                        ...
                        make_handler(cache, cache2, singles2),
                        cb2);
    }
    ...
    this._poll = function ()
    {
        ...
        function (err, update)
        {
            ...
            function (bucket, next)
            {
                ...
                if (buffertools.equals(update.slice(start, end),
                                       ths._last_update.slice(start, end)))
                {
                    ...
                    /*jslint unparam: true */
                    handle(singles[bucket], cache, cache, make_info_handler,
                    function (cache2, singles2)
                    {
                        singles[bucket] = singles2;
                        next();
                    });
...
@param {Function} cb Function to call once the subscription has been registered. This will be passed the following argument:
    ...
    - `{Object} err` If an error occurred then details of the error, otherwise `null`.
*/
QlobberFSQ.prototype.subscribe = function (topic, options, handler, cb)
{
    ...
    function collect(stream, info, cb)
    {
        var chunks = [],
            len = 0;
        ...
        stream.once('end', function ()
        {
            ...
            //process.nextTick(function ()
            //{
                handler(Buffer.concat(chunks, len), info, cb);
            //});
        });
    ...
    {
        handler[this._dehnd] = handler[this._dehnd] || collect;
        collect.accept = handler.accept;
        this._matcher.add(topic, handler[this._dehnd]);
    }
...
@param {String} [topic] Which messages you're no longer interested in receiving via the `handler` function. This should be a topic you've previously passed to [`subscribe`](#qlobberfsqprototypesubscribetopic-options-handler-cb). If topic is `undefined` then all handlers for all topics are unsubscribed.

@param {Function} [handler] The function you no longer want to be called with messages published to the topic `topic`. This should be a function you've previously passed to [`subscribe`](#qlobberfsqprototypesubscribetopic-options-handler-cb). If you subscribed `handler` to a different topic then it will still be called for messages which match that topic. If `handler` is undefined, all handlers for the topic `topic` are unsubscribed.
...
QlobberFSQ.prototype.unsubscribe = function (topic, handler, cb)
{
    ...
    {
        this._matcher.remove(topic, handler[this._dehnd] || handler);
    }
...
@return {Boolean} Whether new messages will be detected.
*/
QlobberFSQ.prototype.is_watching = function ()
{

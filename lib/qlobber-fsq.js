...
function QlobberFSQ(options)
{
    ...
    if (options.disruptor && (options.disruptor.elementSize > 2))
    {
        this._disruptor = options.disruptor;
        this._disruptor_element_size = this._disruptor.elementSize;
    }
    ...
    this._poll = function ()
    {
        ...
        function process_messages(update)
        {
            ...
            async.ensureAsync(function (bucket, next)
            {
                ...
                process_pending(bucket, extra_matcher, next);
            }), polled);
        }

        // TODO: Do we need to force a refresh from disk once in a while?
        // or at least delete expired (or maybe we don't need cached any more?
        // - just go to disk if we need to match existing?)
        // Can we get rid of cache when using disruptor
        // Only go to disk for existing and expired
        // Also, why are we getting them polled 1s apart?
        if (ths._disruptor && (caches.size > 0))
        {
            var bufs = ths._disruptor.consumeNewSync();
            return async.eachSeries(bufs, function (buf, next)
            {
                var n = buf.length / ths._disruptor_element_size;

                async.timesSeries(n, async.ensureAsync(function (i, next)
                {
                    var start = i * ths._disruptor_element_size,
                        len = buf.readUInt16LE(
                            start + ths._disruptor_element_size - 2, true);
                    ...
                    var relpath = buf.toString('utf8', start, start + len),
                        bucket_fmt_fname = relpath.split(path.sep),
...
QlobberFSQ.prototype.publish = function (topic, payload, options, cb)
{
    ...
    function disruptor_commit(start, end)
    {
        if (!ths._disruptor.produceCommitSync(start, end))
        {
            return setImmediate(function ()
            {
                disruptor_commit(start, end);
            });
        }
    ...
    function disruptor_claim()
    {
        var b = ths._disruptor.produceClaimSync();

        if (b.length === 0)
        {
            return setImmediate(disruptor_claim);
        }

        var size = b.length - 2,
            len = b.write(bucket_fmt + path.sep + fname);

        if (len > size)
        {
            ths._error(new Error('buffer too short'));
            b.writeUInt16LE(0, size, true);
            return update();
        }

        b.writeUInt16LE(len, size, true);

        disruptor_commit(ths._disruptor.prevClaimStart,
                         ths._disruptor.prevClaimEnd);
    }

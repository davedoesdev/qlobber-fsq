...
function QlobberFSQ(options)
{
    ...
    function process_all_readdir(bucket, extra_matcher, cb)
    {
    ...
    function process_messages(update)
    {
        ...
        async.timesLimit(num_buckets, ths._bucket_concurrency,
        async.ensureAsync(function (bucket, next)
        {
            // If we wanted to process only pending messages we would call
    ...
    {
        return bucket_formats.get(b % num_buckets);
    };
    ...
    {
        bucket_formats.set(i, format_bucket(i));
    }
    ...
    process.nextTick(function ()
    {
        ...
        if (options.getdents_size > 0)
        {
            try
            {
                process_all = ths._require_getdents('./process_all_getdents')
                    .call(ths,
            ...
            {
                process_all = process_all_readdir;

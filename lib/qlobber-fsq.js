...
function QlobberFSQ(options)
{
    ...
    function make_info_handler(cache, cache2, pending2, extra_matcher)
    {
        return function (info, next)
        {
            ...
            if (info.single)
            {
                ...
                return ths.filter(info2, handlers, function (err, ready, handlers)
                {
                    ths._error(err);
            ...
            ths.filter(info2, handlers, function (err, ready, handlers)
            {
                ths._error(err);
    ...
    function handle(infos, cache, cache2, make_handler, extra_matcher, cb)
    {
        var pending2 = [];
        ...
        function (err, update)
        {
            ...
            async.ensureAsync(function (bucket, next)
            {
                        ...
                        ths._last_update.slice(start, end)))
                {
                    ...
                    handle(extra_matcher ? cache.values() : pending.get(bucket),
                           cache,
                           cache,
                           make_info_handler,
                           extra_matcher,

...
var stream = require('stream'),
    ...
    fs = require('fs'); // graceful-fs readdir sorts so is slower
...
function QlobberFSQ(options)
{
    ...
    process.nextTick(function ()
    {
        ...
        if (ths._do_single)
        {
            try
            {
                ths._fs = ths._require_fs('graceful-fs').gracefulify(
                        ths._require_fs('fs-ext'));
            }
            catch (err)
            {
                ths._fs = ths._require_fs('graceful-fs');

...
function QlobberFSQ(options)
{
    ...
    this._multicast_address = options.multicast_address;
    this._multicast_port = options.multicast_port;

    this._refresh_ttl = options.refresh_ttl === undefined ?
        (options.multicast_address && options.multicast_port ? 10 * 1000 : 0) :
        options.refresh_ttl;

    this._uid_bytes = options.uid_bytes || 16;
    this._peers = new Set();
    ...
    function call_handlers2(handlers, info, cb)
    {
        ...
        function (err, fd)
        {
            function read2()
            {
                ...
                hcb.num_handlers = ths._do_dedup ? handlers.size : handlers.length;
    ...
    {
        handler_queue = async.queue(async.ensureAsync(function (task, cb)
        {
    ...
    function make_info_handler(cache, cache2, pending2, extra_matcher)
    {
        return function (info, next)
        {
            ...
            else
            {
                if (extra_matcher)
                {
                    ...
                    if (info._extra_handlers)
                    {
                        if ((info._matcher_marker === ths._matcher_marker) &&
                            !has_extra_handlers)
                        {
                            extra_handlers = info._extra_handlers;
                            has_extra_handlers = true;
                        }
                        else
                        {
                            handlers = ths._matcher.match(info.topic);
                            for (h of info._extra_handlers)
                            {
                                if (ths._do_dedup)
                                {
                                    if (handlers.has(h))
                        ...
                        delete info._extra_handlers;
                        delete info._matcher_marker;
                    }
                }
            ...
            filter(info2, handlers, function (err, ready, handlers)
            {
                ...
                if (has_extra_handlers)
                {
                    ths._ensure_extra_matcher();
                    info._extra_handlers = extra_handlers;
                    info._matcher_marker = matcher_marker;
                }
    ...
    function fname_to_info(fname, cache, cache2, cb)
    {
        ...
        parse_fname(cache2.bucket_fmt, fname, cb);
    }

    function make_fname_handler(cache, cache2, pending2, extra_matcher)
    {
        var info_handler = make_info_handler(cache, cache2, pending2, extra_matcher);
    ...
    function handle(infos, cache, cache2, pending2, make_handler, extra_matcher, cb)
    {
        function cb2()
        {
            if (ths._chkstop()) { return; }
        ...
        function cb3()
        {
            async.eachLimit(infos,
                            ...
                            async.ensureAsync(make_handler(
                                    cache, cache2, pending2, extra_matcher)),
                            cb2);
        }
        ...
        function cb4()
        {
            ...
            {
                return async.sortBy(infos, function (info, next)
                {
                    process.nextTick(function ()
                    {
                        next(null, info.expires);
                    });
                }, function (err, sinfos)
                {
                    infos = sinfos;
    ...
    function process_pending(bucket, extra_matcher, cb)
    {
        var cache = caches.get(bucket),
            infos = extra_matcher ?
                        // 1. Ensure pending picked up as existing
                        // 2. Ensure pending are processed (otherwise
                        //    any pending not in cache will be lost)
                        wu.chain(cache.values(), pending.get(bucket)) :
                        pending.get(bucket),
            pending2 = [];
    ...
    function process_all(bucket, extra_matcher, cb)
    {
        ...
        function (err, files)
        {
            ...
            last_refreshed.set(bucket, Date.now());
    ...
    function check_update(update, bucket, extra_matcher, cb)
    {
        ...
        if (update.slice(start, end).equals(ths._last_update.slice(start, end)))
        {
            //console.log('no update', require('os').hostname(), bucket);
            return process_pending(bucket, extra_matcher, cb);
        }

        //console.log('update', require('os').hostname(), bucket);
        process_all(bucket, extra_matcher, function (err)
        {
            if (!err)
            {
    ...
    this._poll = function ()
    {
        ...
        var refresh_expired = new Set(),
            now = Date.now();

        if (ths._refresh_ttl > 0)
        {
            for (var i = 0; i < num_buckets; i += 1)
            {
                if ((now - last_refreshed.get(i)) >= ths._refresh_ttl)
                {
                    refresh_expired.add(i);
                }
            }
        }
        ...
        function process_messages(update)
        {
            ...
            async.ensureAsync(function (bucket, next)
            {
                if ((ths._refresh_ttl === 0) || refresh_expired.has(bucket))
                {
                    return check_update(update, bucket, extra_matcher, next);
                }

                return process_pending(bucket, extra_matcher, next);
            }), polled);
        }

        if ((ths._refresh_ttl === 0) || (refresh_expired.size > 0))
        {
            return read_update(process_messages);
        }
    ...
    function create_multicast_sender()
    {
        ths._multicast_sender = dgram.createSocket(
        {
            type: 'udp4',
            reuseAddr: true
        });

        ths._multicast_sender.on('error', emit_error);

        ths._multicast_sender.bind(ths._multicast_port, '127.0.0.1', function ()
        {
            if (ths._chkstop()) { return; }

            this.setMulticastTTL(0);
            this.setMulticastLoopback(false);

            var uid = crypto.randomBytes(ths._uid_bytes).toString('hex'),
                server = unix_dgram.createSocket('unix_dgram');

            // TODO: need to close server when stop

            server.on('error', emit_error);

            server.on('message', function (relpath)
            {
                var bucket_fmt_fname = relpath.toString().split(path.sep),
                    bucket_fmt = bucket_fmt_fname[0],
                    fname = bucket_fmt_fname[1],
                    bucket = parseInt(bucket_fmt, ths._bucket_base);

                parse_fname(bucket_fmt, fname, function (info)
                {
                    pending.get(bucket).push(info);
                    ths.refresh_now();
                });
            });
            ...
            {
                ths._multicast_sender.send(uid,
                                           ths._multicast_port,
                                           ths._multicast_address);
            
            });

            server.bind('\0/qlobber-fsq/' + uid);
        });
    }

    function create_multicast_receiver()
    {
        ths._multicast_receiver = dgram.createSocket(
        {
            type: 'udp4',
            reuseAddr: true
        });

        ths._multicast_receiver.on('error', emit_error);

        ths._multicast_receiver.on('message', function (uid)
        {
            var client = unix_dgram.createSocket('unix_dgram');

            client.setMaxListeners(0);

            client.on('error', emit_error); // TODO: need to remove on error and stop
            ...
            client.connect('\0/qlobber-fsq/' + uid.toString());
        });

        ths._multicast_receiver.bind(ths._multicast_port, ths._multicast_address, function ()
        {
            if (ths._chkstop()) { return; }

            this.addMembership(ths._multicast_address, '127.0.0.1');
    ...
    function start_notifiers(err)
    {
        ...
        if (ths._multicast_address && ths._multicast_port)
        {
            return create_multicast_receiver();
        }
...
QlobberFSQ.prototype.publish = function (topic, payload, options, cb)
{
    ...
    bucket_fmt = this._format_bucket(hasher(
    ...
    bucket = this._bucket_base > 1 ? parseInt(bucket_fmt, this._bucket_base) : 0;
    ...
    function done(err)
    {
        ...
        var info = {
            ...
            size: Math.max(stream.bytesWritten - 1, 0)
        };
    ...
    function rename_and_update()
    {
        ths._fs.rename(staging_fname, msg_fname, function (err)
        {
            ...
            register(update_stream, function ()
            {
                if (ths._multicast_sender)
                {
                    var msg = new Buffer(bucket_fmt + path.sep + fname);

                    function send_to_peer(peer)
                    {
                        peer.send(msg, function (err)
                        {
                            if (err && (err.code === 1))
                            {
                                peer.once('writable', function ()
                                {
                                    send_to_peer(peer);
                                });
                            }
...
QlobberFSQ.prototype.stop_watching = function (cb)
{
    ...
    if (!this._multicast_sender)
    {
        return this._stop_watching(cb);
    }

    var ths = this;

    this._multicast_sender.close(function ()
    {
        if (!ths._multicast_receiver)
        {
            return ths._stop_watching(cb);
        }

        ths._multicast_receiver.close(function ()
        {
            ths._stop_watching(cb);
        });

        ths._multicast_receiver = null;
    });

    this._multicast_sender = null;
};
